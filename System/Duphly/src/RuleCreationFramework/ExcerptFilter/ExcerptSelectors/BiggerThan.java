 ...
 * @author Gaston
 */
public class BiggerThan extends ExcerptConditionSelector{
    Integer note;
    Integer octave;
    Integer quantity;
    Integer grade;
    public BiggerThan(Integer note, Integer octave,Integer quantity) {
        this.note = note;
        this.octave = octave;
        this.quantity= quantity;
        this.grade = null;
    }
    public BiggerThan(Integer grade ,Integer quantity) {
        this.note = null;
        this.octave = null;
        this.grade = grade;
        this.quantity= quantity;
   ...
   @Override
       ...
       int endPosition = Util.calculateNotePositionInListByTimeSum(melody, end);
        ...
        for (int i = startPosition; i < endPosition; i++) {
            ...
            for(int j = i; j < endPosition && j< i+this.quantity; j++){
                if(satisfiesSmaller(melody.get(j), Util.LookForBaseChord(base, melody, j)))
                    count ++;
                else{
                    i = j;
                    j = endPosition;
                ...
                if(count >= quantity && !this.sharesPartition(i, i+this.quantity, alreadySelected)){
                    Pair<Double,Double> pair = new Pair<Double, Double>(Util.calculateTimeSumByPosition(melody, i),Util.calculateTimeSumByPosition(melody, i + this.quantity));
                    solution.add(pair);
                }
            }
    ...
    private boolean satisfiesSmaller(Note n, Chord c){
        ...
        if(this.grade == null){
            ...
            solution = this.grade > Util.calculateNoteGradeForChord(c, n);
        return solution;
    }
