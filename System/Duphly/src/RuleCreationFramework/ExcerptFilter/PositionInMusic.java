...
public class PositionInMusic {
  ...
  public Pair<Double, Double> GetPosition(
    ...
    if (this.isPercentaje) position =
      this.GetPositionByPercentaje(melody, melodyStart, melodyEnd, length);
    if (this.aleatorio) position =
      this.GetRandomPosition(melody, melodyStart, melodyEnd, length);
    if (this.start || this.middle || this.end) position =
      this.GetPositionByRelativePosition(
          melody,
          melodyStart,
          melodyEnd,
          length
    ...
    if (this.greaterThan || this.lesserThan) position =
      this.GetPositionByNumber(melody, melodyStart, melodyEnd, length);
    return position;
  }

  private Pair<Double, Double> GetPositionByPercentaje(
    List<Note> melody,
    double melodyStart,
    double melodyEnd,
    int length
    ...
    int percentajePosition = (int) Math.floor(
      noteQuantity * this.specificNumber / 100
    );
    int partitionPositionStart = -1;
    int partitionPositionEnd = -1;
    partitionPositionStart =
      percentajePosition - length / 2 >= startPosition
        ? percentajePosition - length / 2
        : percentajePosition;
    partitionPositionEnd =
      percentajePosition + length / 2 >= endPosition
        ? percentajePosition + length / 2
        : percentajePosition;
    if (
    ...
    double start = Util.calculateTimeSumByPosition(
      melody,
      partitionPositionStart
    );
  ...
  private Pair<Double, Double> GetPositionByNumber(
    List<Note> melody,
    double melodyStart,
    double melodyEnd,
    int length
    ...
    if (this.greaterThan == true) {
      partitionPositionStart =
        startPosition + this.relativePosition > endPosition
          ? startPosition
          : startPosition + this.relativePosition;
      partitionPositionEnd =
        (
          startPosition + this.relativePosition + length > endPosition
            ? endPosition
            : startPosition + this.relativePosition + length
        );
    }
    double start = Util.calculateTimeSumByPosition(
      melody,
  ...
  private Pair<Double, Double> GetPositionByRelativePosition(
    List<Note> melody,
    double melodyStart,
    double melodyEnd,
    int length
