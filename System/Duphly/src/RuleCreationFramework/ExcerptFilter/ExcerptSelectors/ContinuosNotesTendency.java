...
public class ContinuosNotesTendency extends ExcerptConditionSelector{
    ...
    public List<Pair<Double, Double>> applySelector(List<Note> melody, List<Chord> base, double start, double end, int length, List<Pair<Double, Double>> alreadySelected) {
        ...
        for (int i = startPosition+1; i < endPosition; i++) { // puede llegar a haber problema de boundaries.
            if(this.tendencyContinues(tendencyUp, melody.get(i), melody.get(i+1)))
                ...
                if(this.sharesPartition(intervalStart, i, alreadySelected)&& count >= this.quantity)
    ...
    private boolean tendencyContinues(boolean up, Note present, Note next){
        return (up && Util.getExpandedFormNote(present.getNote(), present.getOctave())<Util.getExpandedFormNote(next.getNote(), next.getOctave())) 
                || (!up && Util.getExpandedFormNote(present.getNote(), present.getOctave())>Util.getExpandedFormNote(next.getNote(), next.getOctave()));
    }
}
