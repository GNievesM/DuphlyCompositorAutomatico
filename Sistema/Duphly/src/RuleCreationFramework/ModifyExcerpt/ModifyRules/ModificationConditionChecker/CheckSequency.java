...
public class CheckSequency extends ModificationCondition{
    ...
    public CheckSequency(boolean isASequency, boolean asc, Integer number, boolean allowRepeat) {
    ...
    public boolean checkCondition(List<Chord> base, List<Note> melody, Double start, Double end) {
       ...
       for (int i = positionStartIndex; i < positionEndIndex-1; i++) {
           ...
           if(apliesConditions(n1,n2))
               correctNotes+=1;
           else
               correctNotes =0;
           if(correctNotes == this.number)
               return true&&this.isASequency;
       }
    ...
    public List<Double> listFaultyPositions(List<Chord> base, List<Note> melody, Double start, Double end) {
       ...
       for (int i = positionStartIndex; i < positionEndIndex-1; i++) {
           ...
           Note n2 = melody.get(i+1);
           if(apliesConditions(n1,n2))
               faulty.add(Util.calculateTimeSumByPosition(melody, i));
