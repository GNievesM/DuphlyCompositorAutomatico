...
public class GuaranteeDifferenceBetweenNotes extends GuaranteedCondition {
  ...
  public GuaranteeDifferenceBetweenNotes(
    List<Chord> base,
    List<Note> melody,
    Double start,
    Double end,
  ...
  private int targetNumber() {
    if (comp.esMayor()) return diffNumber + 1;
    if (comp.esMenor()) return diffNumber - 1;
  ...
  protected double calculateDifferenceSum() {
    ...
    if (this.sumResult == null) this.calculateTotalSum(
        notePositionStart,
        notePositionEnd,
        melody
      );
  ...
  protected List<Pair<Integer, Integer>> calculateFaultyPartition() {
    ...
    if (sumResult == null) this.calculateTotalSum(
        notePositionStart,
        notePositionEnd,
        melody
      );
    ...
    for (int i = notePositionStart; i < notePositionEnd - 1; i++) {
      ...
      int diff =
        ...
        Util.getExpandedFormNote(toCompare.getNote(), toCompare.getOctave());
      if (
        (totalSumAfterFaulty > solve && diff < 0) ||
        (totalSumAfterFaulty < solve && diff > 0)
      ) {
        Pair<Integer, Integer> faulty = new Pair<Integer, Integer>(
          new Integer(i),
          new Integer(i + 1)
        );
      ...
      if (satisfiesCondition(totalSumAfterFaulty)) return particiones;
    }
