 ...
 * @author gasto_000
 */
public class ChordRule3b extends GenericChordRule {
    ...
    protected List<Chord> seekChordPlace(List<Chord> base, int tempo) {
        ...
        for (int i = 0; i < base.size() - 1; i++) {//No se puede aplicar la regla sobre el ultimo acorde
            if (sumaTiempos == tempo && rule3bPrecondition(base.get(i),base.get(i+1),tempo)){//&& base.get(i).GetSeptima() == false && base.get(i + 1).GetSeptima() == true && VerifyRoot(tempo, base.get(i).GetGrade())) {
    ...
    private Chord TransformChord(Chord c1) {
        ...
        chord1.setMinor(false);
        chord1.setFlat(c1.isFlat());
        return chord1;

    }
    
    private boolean rule3bPrecondition(Chord c1, Chord c2,int tempo){
        return !c1.GetSeptima()&& !c1.isMinor()&& VerifyRoot(tempo, c1.GetGrade()) && c2.GetSeptima() && c2.isMinor();     
    }
