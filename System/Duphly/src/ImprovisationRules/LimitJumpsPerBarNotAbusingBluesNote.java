 ...
 * @author gasto_000
 */
public class LimitJumpsPerBarNotAbusingBluesNote extends GenericImprovisationRule {
    ...
    private void NoteListFormationForChord(double duration, Chord acord, int jumpsLeft, boolean firstTime) {
      ...
      while(!noteSelected){
            int NoteToImprovisation = Util.RandomizePitchNotAbusingBluesNote();// 6 por trabajar con la escala del blues(pentatonica)
            ...
            switch (NoteToImprovisation) {
                case 0:
                    ...
                    break;
                case 1:
                    note =  Util.CalculateNoteHeightForChord(acord, 0);
                    if (firstTime && (Math.abs(noteBehind - note) < 4) && octave == noteOctave) {
                        this.improvisation.add(new Note(duration, note, noteOctave, 'b'));
                          noteSelected= true;
                        break;
