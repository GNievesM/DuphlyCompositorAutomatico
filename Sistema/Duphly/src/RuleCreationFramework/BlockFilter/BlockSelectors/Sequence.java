    ...
    public List<Pair<Integer, Integer>> applySelector(List<Note> melody, List<Chord> base, Integer start, Integer end, Integer blockQuantity) {
        ...
        for(int i =start;i<end;){
            ...
            for(int j = indexStart; j<indexEnd; j++){
                if((pitches!= null && pitches.get(sequenceIndex) == melody.get(j).getNote())
                   || notes != null && notes.get(sequenceIndex).getNote()== melody.get(j).getNote() && notes.get(sequenceIndex).getOctave()== melody.get(j).getOctave()){
                    correctSequence ++;
                    sequenceIndex ++;                    
                }else{
                    correctSequence =0;
                ...
                if((pitches!=null && correctSequence == this.pitches.size())||
                     notes != null && correctSequence == this.notes.size()){
                    j = indexEnd;
            ...
            if((this.pitches != null && correctSequence != this.pitches.size()) || this.notes != null && correctSequence != this.notes.size())
                i++;
