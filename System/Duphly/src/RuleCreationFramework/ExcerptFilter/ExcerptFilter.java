...
public class ExcerptFilter {
  ...
  public List<Note> applyFilter(
    ...
    List<Pair<Double, Double>> filtered = applyFilters(
      base,
      modifiedMelody,
      start,
      end
    ...
    for (ModifyExcerpt me : modify) {
      for (Pair<Double, Double> par : filtered) {
        modifiedMelody =
          me.Modify(base, melody, par.getKey(), par.getValue(), style);
      }
    }
    return modifiedMelody;
  }

  private List<Pair<Double, Double>> applyFilters(
    List<Chord> base,
    List<Note> melody,
    Double start,
    Double end
