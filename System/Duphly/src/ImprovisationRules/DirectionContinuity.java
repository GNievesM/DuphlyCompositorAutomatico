...
public class DirectionContinuity extends GenericImprovisationRule {
    ...
    public List<Note> GenerateImprovisation(List<Chord> base) {
        ...
        for (int i = 0; i < r.getRhythmSequence().size() - 1; i++) {
            ...
            int actualNoteExpanded = Util.getExpandedFormNote(improvisation.get(i).getNote(), (improvisation.get(i).getOctave()));
            boolean found = false;
            while (!found ) {
                if (goingUp) {
                    actualNoteExpanded++;
                ...
                Note n = new Note(r.getRhythmSequence().get(i + 1).getDuration(), Util.getSmallFormNotePitch(actualNoteExpanded), Util.getSmallFormNoteOctave(actualNoteExpanded), 'n');
                int h = Util.IdentifyHeightSemiToneInChord(r.getRhythmSequence().get(i + 1).getChord(), n);
                if (h != 0) {
                    improvisation.add(Util.CalculateNoteForChord(r.getRhythmSequence().get(i + 1).getChord(), h, r.getRhythmSequence().get(i + 1).getDuration(),Util.getSmallFormNoteOctave(actualNoteExpanded)));
                    found = true;
                }else 
                    if(n.getOctave()<6 || n.getOctave()>9){ 
                        improvisation.add(Util.CalculateNoteForChord(r.getRhythmSequence().get(i+1).getChord(), 3, r.getRhythmSequence().get(i+1).getDuration(), 6));
                    found = true;}
    ...
    private List<Note> AnotherImplementation(List<Chord> base, List<Note> improvisation) {
        ...
        for (int i = 0; i < improvisation.size() - 1; i++) {
            if (total == this.continuityConstant) {
                ...
                if (Util.IsHIgherOrEqualPitchThan(improvisation.get(i), improvisation.get(i + 1)) && goingUp == true && improvisation.get(i+1).getOctave()<=8) {
                ...
                if (Util.IsHIgherOrEqualPitchThan(improvisation.get(i + 1), improvisation.get(i)) && goingUp == false && improvisation.get(i+1).getOctave()>=5) {
                    Note n = improvisation.get(i + 1);
