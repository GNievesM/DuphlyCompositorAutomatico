...
public class GuaranteeSequence
  ...
  public boolean checkCondition(
    ...
    if (c == null) c =
      new CheckSequency(
        true,
        asc,
        Util.calculateNotePositionInListByTimeSum(melody, start) -
        Util.calculateNotePositionInListByTimeSum(melody, end),
        allowRepeat
      );
  ...
  protected double calculateDifferenceSum() {
    ...
    for (int i = notePositionStart; i < notePositionEnd - 1; i++) {
      ...
      if (this.apliesConditions(n1, n2)) {
        notesAligned++;
        if (notesAligned > maxAligned) maxAligned = notesAligned;
      } else {
        notesAligned = 0;
      ...
      if (
        this.number != null &&
        notesAligned == this.number ||
        notesAligned == notePositionEnd - notePositionStart - 1
      ) return 100;
    }
