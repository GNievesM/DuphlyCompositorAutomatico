...
public class BlockPositionInMusic {
    ...
    public Pair<Integer, Integer> GetPosition(List<Note> melody, int blockQuantity) {
       Pair<Integer,Integer> position=null;
        if(this.isPercentaje)
            position = this.GetPositionByPercentaje(melody, blockQuantity);
        if(this.aleatorio)
            position = this.GetRandomPosition(melody, blockQuantity);
        if(this.start || this.middle || this.end)
            position = this.GetPositionByRelativePosition(melody, blockQuantity);
        if(this.greaterThan || this.lesserThan)
            position = this.GetPositionByNumber(melody, blockQuantity);
        return position;
    }
    
    
    private Pair<Integer,Integer> GetPositionByPercentaje(List<Note> melody, int blockQuantity){
     
        int totalBlocks = Util.GetBlockQuantity(melody);
        
        int percentajePosition = (int) Math.floor(totalBlocks * this.specificNumber /100);
        int partitionPositionStart=-1;
        int partitionPositionEnd=-1;
        partitionPositionStart = percentajePosition - blockQuantity/2 >= 0 ? percentajePosition-blockQuantity/2 :percentajePosition;
        partitionPositionEnd = percentajePosition + blockQuantity/2 >= totalBlocks ? percentajePosition+blockQuantity/2 :percentajePosition;
        ...
        Integer start =Util.getBlockPositionByNumber(melody,partitionPositionStart);
        Integer end =Util.getBlockPositionByNumber(melody, partitionPositionEnd);// incluye el tiempo correspondiente de la primer nota del siguietne compas.
        Pair<Integer,Integer> solution = new Pair<>(start,end);
        return solution;
    }
    
    private Pair<Integer,Integer> GetPositionByNumber(List<Note> melody, int blockQuantity){
        int totalBlocks = Util.GetBlockQuantity(melody);
        int partitionPositionStart=-1;
        int partitionPositionEnd=-1;
        if(this.greaterThan==true){
            partitionPositionStart = this.relativePosition;
            partitionPositionEnd =  this.relativePosition+blockQuantity > totalBlocks ? totalBlocks :this.relativePosition+blockQuantity;
        }
    ...
    private Pair<Integer,Integer> GetPositionByRelativePosition(List<Note> melody,int blockQuantity){
    ...
    private Pair<Integer,Integer> GetRandomPosition(List<Note> melody,int blockQuantity){
