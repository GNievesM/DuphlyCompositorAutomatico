...
public class SpecificGrade extends ExcerptConditionSelector{
    ...
    public SpecificGrade(Comparador gradeComp, Integer grade, Integer quantity, Comparador quantityComp) {
    ...
    public List<Pair<Double, Double>> applySelector(List<Note> melody, List<Chord> base, double start, double end, int length, List<Pair<Double, Double>> alreadySelected) {
      ...
      for(int i= startPosition, intervalStart = i,correct=0; i < endPosition;i++){          
           Note present = melody.get(i);
           
           int grade = Util.calculateNoteGradeForChord(Util.LookForBaseChord(base, melody, i), present);
           if(grade ==this.grade && this.gradeComp.esIgual()||
             grade < this.grade && this.gradeComp.esMenor() ||
             grade > this.grade && this.gradeComp.esMayor()){
               // if(intervalStart !=startPosition){
                   correct ++;
                //}
            }else {
                 if(correct > this.quantity && this.quantityComp.esMayor() 
                 || correct < this.quantity && this.quantityComp.esMenor()
                 ||correct == this.quantity && this.quantityComp.esIgual()){
