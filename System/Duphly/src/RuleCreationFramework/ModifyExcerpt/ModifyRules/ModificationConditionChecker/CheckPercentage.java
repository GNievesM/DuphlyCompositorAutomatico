...
public class CheckPercentage extends ModificationCondition {
    ...
    public CheckPercentage(Note note, boolean absolut, int percentage, Comparador comp){
        this.note = note;
    ...
    public boolean checkCondition(List<Chord> base, List<Note> melody, Double start, Double end) {
        ...
        int totalPercentage = quantity*100/(positionStartIndex-positionEndIndex);
        return totalPercentage > this.percentage && cmp.esMayor() || totalPercentage<this.percentage&& cmp.esMenor() || totalPercentage == this.percentage && cmp.esIgual();
    }
