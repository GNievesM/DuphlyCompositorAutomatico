 ...
 * @author gasto_000
 */
public class ChordRule6a extends GenericChordRule {
    ...
    protected List<Chord> seekChordPlace(List<Chord> base, int tempo) {
        ...
        for (int i = 0; i < base.size() - 2; i++) {//No se puede aplicar la regla sobre el ultimo acorde ni el penultimo
            if (sumaTiempos == tempo && rule6aPrecondition(base.get(i),base.get(i+1), base.get(i+2))){//&& base.get(i).GetSeptima() == false && base.get(i + 1).GetSeptima() == true && VerifyRoot(tempo, base.get(i).GetGrade())) {
    ...
    private boolean rule6aPrecondition(Chord c1, Chord c2,Chord c3){
        return !c1.GetSeptima()&& !c2.GetSeptima()&& !c3.GetSeptima() &&
               c1.isMinor() == c2.isMinor()&&
                c1.GetGrade()== c2.GetGrade() && ChordOperationUtil.isDominantOf(c3, c1);     
    }
