...
public class DashedDirectionContinuity extends GenericImprovisationRule {
  ...
  public Pair<List<Note>, List<Chord>> VerifyAndCorrectImprovisation(
    ...
    Pair<List<Note>, List<Chord>> par = new Pair<List<Note>, List<Chord>>(
  ...
  private List<Note> AnotherImplementation(
    ...
    for (int i = 0; i < improvisation.size() - 1; i++) {
      if (total == this.continuityConstant) {
        ...
        ) goingUp = false; else goingUp = true;
      } else {
        if (improvisation.size() < i + dashSize + 1) i += dashSize;
        if (
          Util.IsHIgherOrEqualPitchThan(
            improvisation.get(i),
            improvisation.get(i + 1)
          ...
          switch (notePitch - Util.CalcChord(c)) {
            case 0:
              notePitch = (extendedNotationNote + 3) % 12;
              noteOctave = (extendedNotationNote + 3) / 12;
              break;
            case 3:
              notePitch = (extendedNotationNote + 2) % 12;
              noteOctave = (extendedNotationNote + 2) / 12;
              break;
            case 5:
              notePitch = (extendedNotationNote + 1) % 12;
        ...
        //i++;
        if (total == 0) total = this.continuityConstant;
      }
    }
